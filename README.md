...
2. **유닛 테스트**
    ...
    - Ex. 특정 함수가 입력 값을 받아 올바른 결과를 반환하는지 테스트
3. **통합 테스트**
    - 여러 유닛이 함께 작동할 때 발생하는 문제를 찾음
    - Ex. 로그인 기능이 사용자 데이터베이스와 올바르게 통신하는지 확인
4. **모의 객체**
    - 실제 운영 환경 대신 모의 객체를 사용하여 테스트
    - Ex. 실제 서버 대신 모의 서버를 사용하여 API 호출을 테스트
...
1. **베타 테스트**
    ...
    - Ex. 제한된 사용자 그룹에게 앱을 배포하여 피드백을 수집
2. **사용자 테스트**
    - 최종 사용자가 실제 시나리오에서 앱을 테스트하여 요구사항이 충족되는지 확인
    - Ex. 클라이언트가 앱의 각 기능을 사용해보고, 요구사항에 맞게 동작하는지 확인
3. **성능 테스트**
    - 앱이 다양한 조건에서 얼마나 잘 작동하는지 테스트
    - Ex. 많은 사용자가 동시에 로그인할 때 서버 응답 시간이 어떻게 변하는지 확인
4. **부하 테스트**
    - 앱이 높은 부하에서도 안정적으로 작동하는지 테스트
    - Ex. 많은 사용자 요청이 동시에 들어왔을 때 시스템이 견딜 수 있는지 확인
5. **실제 데이터 사용**
    - 테스트 환경이 아닌 실제 데이터와 실제 서버를 사용하여 테스트
    - Ex. 실제 사용자 계정을 사용하여 로그인, 데이터베이스 조회, 데이터 입력 등을 테스트
...
- **Navihost, Navigate, ViewModel :**  [Jetpack Compose SampleApp](https://github.com/android/compose-samples/tree/main) 의 [Jetnews](https://github.com/android/compose-samples/tree/main/JetNews) 프로젝트를 모티브로 하여 개발
    ...
    - 사유 : Jetpack Sample 앱은 최신 모범 사례를 따릅니다. Compose 를 어떻게 효과적으로 사용할 수 있는지 배울 수 있습니다. 다양한 기능을 예시를 통해 배울 수 있습니다. 
    EX) 상태 관리, 화면 이동, 애니메이션 등
- **API 통신** : `okhttp`, `retrofit` 라이브러리 사용
    - 사유 : 안드로이드에서 가장 기본적이고 대중적인 API 라이브러리를 사용 했습니다.
- **이미지** : `coil` 라이브러리 사용
    - 사유 : Glide 보다 Coil 라이브러리가 Jetpack Compose 와 호환성이 높습니다.
- **검색 리스트 UI** : `LazyVerticalStaggeredGrid` 사용하여 2 Cell Grid 형식으로 나타냄
    - 사유 : 이미지 크기에 따라 유동적으로 리스트에 보여줄 수 있습니다.
    이미지가 서로 다른 비율을 가질 때도 자연스럽게 정렬되며, 공간을 효율적으로 사용할 수 있습니다.
- **Theme** : `MaterialTheme`사용, Blue 계열의 Dark / Light 테마 구현
    - 사유 : 앱에서 일관된 스타일을 유지할 수 있고, 테마 변경 할 떄 코드 수정과 같은 번거로움이 없습니다.
